public class LingStreetNetwork {
    int N;  //number of places
    int [][] network; //adjacency matrix

    public LingStreetNetwork(int [][] network){
    ...
    public int nearestNeighbour(){
        ...
        for(int j = 1; j < N; j++){
            if(network[0][j] < 0){ //no direct path
                continue;
            }
    ...
    public static void main(String[] args) {
        int[][] distances = {   //adjacency matrix
            {0, -1, 5, 10},
            {-1, 0, 4, 2},
            {-1, 1, 0, 4},
            {3, -1, 7, 0}
        ...
        LingStreetNetwork prog = new LingStreetNetwork(distances);
        System.out.println("Nearest neighbour: " + prog.nearestNeighbour());
        System.out.println("Shortest to School: " + prog.shortestToSchool());
    }
}
